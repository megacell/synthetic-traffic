...
def generate_small_graph(num_cols=5, num_rows=5, num_routes_per_od_pair=2):
  ...
  G, sensors = construct_grid(n,m)
  G = add_weights(G,n,m)
  G, sensors = add_inverse(G, sensors)

  H = invert_graph_weights(G)
  routes = pairwise_shortest_routes(H, n, m, r)
...
def construct_grid(n,m):
    ...
    for j in range(0, n-1):
        G.add_edge((m-1) * n + j,(m-1) * n + j + 1, weight=1)
...
def pairwise_shortest_routes(H, n, m, r):
